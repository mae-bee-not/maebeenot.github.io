...
DATABASE_PATH = Path(os.getenv("DATABASE_PATH", "/data/tetris.sqlite"))
ALLOWED_ORIGINS = {
    origin.strip()
...
class TetrisHandler(BaseHTTPRequestHandler):
    server_version = "coolbugs-tetris-leaderboard/1.0"
    ...
    def do_POST(self) -> None:
        ...
        record = create_score(name, score)
        self.send_json(HTTPStatus.CREATED, {"score": record})


def main() -> None:
    init_db()
    server = ThreadingHTTPServer((HOST, PORT), TetrisHandler)
    print(f"Tetris leaderboard API listening on {HOST}:{PORT}", flush=True)
