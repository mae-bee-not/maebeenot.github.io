document.addEventListener("DOMContentLoaded", function () {
  ...
  const themeColors = {
    ...
    "gruvbox-rainbow": ["#83a598", "#fe8019", "#fb4934", "#b8bb26", "#fabd2f", "#8ec07c", "#d3869b"],
  };
  ...
  function isValidMove(matrix, cellRow, cellCol) {
    for (let row = 0; row < matrix.length; row++) {
      for (let col = 0; col < matrix[row].length; col++) {
        if (matrix[row][col] && (cellCol + col < 0 || cellCol + col >= playfield[0].length || cellRow + row >= playfield.length || (playfield[cellRow + row] && playfield[cellRow + row][cellCol + col]))) {
          return false;
        }
      }
    }
